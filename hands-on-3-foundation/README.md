...
## Regras do Jogo
1. **Modularização:** O código NÃO pode ser um monólito. Peça ao seu coding agent para gerar arquivos separados para `vpc.tf`, `subnets.tf`, `gateways.tf` e `variables.tf`.
...
> **Transposição (CLI):** `claude "prompt"` (Claude Code), `gemini "prompt"` (Gemini CLI).
> Em IDEs, peça ao seu assistente para criar os arquivos.

**💡 Dica de Fluxo:** Como a IA irá gerar o conteúdo de múltiplos arquivos em uma única resposta, você terá que criar a estrutura de pastas e arquivos manualmente (ex: `mkdir -p modules/vpc`) e colar o conteúdo correspondente gerado pelo coding agent em cada arquivo (`main.tf`, `variables.tf`, etc).
